<template>
  ...
  <div>
    <p class='font-weight-light'>Note: This is an application built on Görli testnet for demonstration purpose only. Do not use assets with real value.
      Assets minted may not have any value. Use at your own risk.</p>
...
<div class ="row">
    ...
    <p><a class=" btn btn-primary text-decoration-none"
         target="_blank"
         href='https://goerli-faucet.slock.it/'>Get some Göeth here</a> or <a class=" btn btn-primary text-decoration-none" target="_blank"
         href='https://faucet.goerli.mudit.blog/'>Here</a> </p>
     </div>
     <div class ="col-md-6">
         <h3> Tutorial </h3>
         <iframe width="560"
                 height="315"
                 src="https://www.youtube.com/embed/pF4FMtcrWfY"
                 frameborder="0"
    ...
    <div class="alert alert-danger" v-if="warning" role="alert">
      Please use an Ethereum compatible browser (e.g. <a href="https://metamask.io/">Metamask</a>) and select Göerli testnet to continue.
    </div>
      ...
      <div v-if="showInfo">
        <div class="row" v-if="assetId">
          ...
          info='This is the ERC20 token contract address for this asset'"
               ...
               :class="{hi: !isHovering3, none: isHovering2|| isHovering1|| isHovering4 || isHovering5 || isHovering6}"><a target="_blank" :href="'https://goerli.etherscan.io/address/'+erc20Address">
                {{erc20Address}}</a> </p>
          </div>
          ...
          info='This is the URL from which the data will be fetched by Razor Oracle Network'"
               ...
               :class="{hi: !isHovering4, none: isHovering1 || isHovering2 || isHovering3 || isHovering5 || isHovering6}"><a :href="selected.url" target="_blank">{{selected.url}} </a>
            </p>
          </div>
          <div class="col-md-2 card-body" v-if="selected">
        ...
        <div class="alert alert-info"
             role="alert"
             :class="{invisible: !(isHovering1 || isHovering2 || isHovering3 || isHovering4 || isHovering5 || isHovering6)}">
          {{info}} </div>
      </div>
        ...
        <div class="row  d-flex p-4 flex-row align-items-stretch">
          ...
          <div class="col-md-12 card">
            <div class="card-body">
              ...
              <p>Create a Collateralized Debt Position with ether and mint synthetic tokens. Tokens will be minted with 500% collateral
                ratio by default.</p>
              <div class="mb-4">
                <label>ETH</label>
        ...
        <div class="col-md-6 ">
          <div class="card">
            <div class="card-body">
              ...
              <h4><small class="display-5 text-muted" v-if="ratioString">Collateral ratio: {{ratioString}}%</small></small>
              </h4>
            </div>
          </div>
        </div>
      ...
      <div class="row  d-flex p-4 flex-row align-items-stretch" v-if="ratioString">
        ...
        <div class="col-md-12 card" v-if="action===2">
          ...
          <div class="card-body">
            ...
            <div class="mb-4">
              <p>Mint more tokens and increase debt without adding collateral. If the Collateral ratio drops below 200%, it may
                be liquidated.</p>
              <label>Amount</label>
  ...
  export default {
    ...
    computed: {
      ...
      CRafterCollateralized: function() {
        ...
        const x = new BN(this.debt)
          .multipliedBy(this.valueOnChainInEth)
          .dividedBy(new BN(this.collateral).plus(this.addEth))
      ...
      CRafterDraw: function() {
        ...
        const x = new BN(this.debt)
          .plus(new BN(this.drawAmount))
          .multipliedBy(this.valueOnChainInEth)
          .dividedBy(new BN(this.collateral))
        return 100 / x
        // return Math.round((100 / x),2)
      }
    },
    ...
    methods: {
      ...
      getJobs: async function() {
        ...
        for (let i = 1; i < data.data.message.length; i++) {
          if (data.data.message[i].result === 0 || data.data.message[i].result === '0') continue
          this.jobs.push({
